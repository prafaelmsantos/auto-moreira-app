...
import { IUserUpdate } from "../../models/identity/User";

export default function AdminUser() {
  const [isLoading, setIsLoading] = useState(false);
  const [users, setUsers] = useState<IUserUpdate[]>([]);
  ...
  useEffect(() => {
    setIsLoading(true);
    const endpoint = `${BASE_API_URL}${"api/users"}`;
    getData<IUserUpdate[]>(`${endpoint}`)
      .then((data) => {
        setUsers(data);
        setIsLoading(false);
      })
